...
const parseOnlyOne = async (inputStream, tagToParse) => {
	strictEqual(typeof tagToParse.tag, 'string', 'tagToParse.tag must be a string')
	ok(tagToParse.tag, 'tagToParse.tag must not be empty')
	let first = true
	const shouldParseNode = (tag) => {
		if (tag === tagToParse.tag && first) {
			first = false
			return true
		}
		return false
