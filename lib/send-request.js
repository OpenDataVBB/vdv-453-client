...
class Vdv453HttpError extends Error {
	constructor(service, call, msg, req, reqOpts, reqBody, res, resBody = null) {
		...
		this.statusMessage = res?.statusMessage ?? null
...
const createSendRequest = (cfg, opt = {}) => {
	...
	// uniquely identifies the server/endpoint we're talking to
	const remoteEndpointId = createHash('sha1')
	.update(endpoint)
	.update(leitstelle)
	.digest('hex')
	.slice(0, 10)
	...
	const sendRequest = async (service, call, rootTag, rootChildren, opt = {}) => {
		...
		const reqOpts = {
			...
			headers: {
				...
				// todo: send user-agent with repo & version!
			},
			...httpRequestOptions,
		...
		const bodyAsTree = x(rootTag, {
			Sender: leitstelle,
			Zst: getZst(),
			// todo: allow passing in additional props, e.g. AboAnfrage.XSDVersionID
		}, rootChildren)
		...
		try {
			...
			if (err.code === 'ABORT_ERR' && err.cause) {
				throw new Vdv453HttpError(
					service,
					call,
					`request aborted: ${err.cause}`,
					req,
					reqOpts,
					reqBody,
					null,
		...
		const assertStatusAntwortOk = (statusAntwort, addLogLabels = {}) => {
			// todo: make err.message more helpful, e.g. by throwing a Vdv453ApiError
			try {
